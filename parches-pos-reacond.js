    ...
    return `<div id="reacondDetalleLote"><div class="nxRcCard reacond-detail-card">
      ...
      <div class="nxRcRow" style="margin:14px 0 10px">
        ...
        <div class="nxRcActs">
          ...
          <button type="button" class="btn nxRcBtn light" style="color:#b91c1c;border-color:#fca5a5" onclick="window.nxRc.imprimirCostos(false,true)" title="Abre la impresión: elige «Guardar como PDF»"><i class="ti ti-file-type-pdf"></i> PDF</button>
...
.nxRcH3{margin:0;font-size:15px;font-weight:800;letter-spacing:-.01em;display:flex;align-items:center;gap:8px}.nxRcH3 .ti{color:var(--rc-gold)}
...
.btn.nxRcBtn{transition:transform 100ms ease-out,background 160ms ease,border-color 160ms ease,color 160ms ease}.btn.nxRcBtn:active{transform:scale(.97)}.btn.nxRcBtn[disabled]{opacity:.55;cursor:default}
...
.reacond-main-tab{display:flex;align-items:center;gap:8px;min-height:40px;padding:4px 10px;border:1px solid transparent;border-radius:13px;background:transparent;color:#5b5951;font:inherit;font-size:12px;font-weight:800;cursor:pointer;text-align:left;min-width:0}
...
.reacond-main-icon{display:inline-flex;align-items:center;justify-content:center;flex:0 0 26px;width:26px;height:26px;border-radius:50%;background:#f3f0e8;color:#806515;border:1px solid var(--rc-line);font-size:15px}
...
.nxRcStages .tab-btn{flex:0 0 auto;display:inline-flex;align-items:center;gap:7px;min-height:36px;padding:4px 10px 4px 5px;border:1px solid transparent;border-radius:999px;background:transparent;color:#5b5951;font:inherit;font-size:12px;font-weight:800;cursor:pointer;white-space:nowrap}
.nxRcStages .tab-btn.active{background:#fff;color:#111;border-color:#c9a227;box-shadow:0 6px 14px -10px rgba(201,162,39,.7)}
.reacond-stage-number{display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;border-radius:50%;background:#fff;color:#85817a;border:1px solid var(--rc-line);font-size:10.5px;font-weight:800}
...
.reacond-kpi-card{padding:10px 8px;border-radius:12px;text-align:center;cursor:pointer;border:2px solid transparent;box-shadow:var(--rc-shadow)}.reacond-kpi-card>div:first-child{font-size:22px;letter-spacing:-.02em;font-weight:800;line-height:1}.reacond-kpi-card>div:last-child{font-size:10.5px;font-weight:700;margin-top:4px;letter-spacing:.03em}
...
.nxRcStepper{display:flex;justify-content:space-between;position:relative;padding:6px 4px}.nxRcStepper .line{position:absolute;left:30px;right:30px;top:22px;height:3px;background:#e2e8f0;z-index:0}.nxRcStepper .fill{position:absolute;left:30px;top:22px;height:3px;background:#10b981;z-index:0;transition:width .3s}
.stepper-step{position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;gap:6px;width:70px;text-align:center;cursor:default}.stepper-circle{width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#fff;border:2px solid #e2e8f0;color:#94a3b8;font-size:16px}.stepper-label{font-size:10.5px;font-weight:700;color:#64748b;line-height:1.2}
...
.pieza-actions{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}.pieza-actions>div{flex:1 1 100%}
.pieza-action-btn{display:inline-flex;align-items:center;justify-content:center;gap:6px;width:auto;min-height:34px;margin:0;padding:0 12px;border-radius:9px;border:1px solid transparent;cursor:pointer;font:inherit;font-size:12.5px;font-weight:700;letter-spacing:-.005em;transition:transform 100ms ease-out,filter 120ms ease}.pieza-action-btn:active{transform:scale(.97)}.pieza-action-btn:hover{filter:brightness(1.04)}
@media(max-width:600px){.pieza-action-btn{flex:1 1 auto}}.pieza-action-btn.entregar{background:linear-gradient(180deg,#2563eb,#1d4ed8);color:#fff}.pieza-action-btn.recibir{background:linear-gradient(180deg,#10b981,#059669);color:#fff}
...
@media(prefers-reduced-motion:reduce){.btn.nxRcBtn,.pieza-action-btn,.reacond-lote-progress>div,.nxRcStepper .fill,.nxRcSpin{transition:none!important;animation:none!important}.btn.nxRcBtn:active,.pieza-action-btn:active{transform:none}}
...
html.nx-studio .modal.nxRcModal input::placeholder,html.nx-studio .modal.nxRcModal textarea::placeholder,html.nx-studio #v-pos .nxRc input::placeholder{text-transform:none;letter-spacing:-.005em;color:#85817a}
...
(function () {
  ...
  async function imprimirLabelEquipo(equipoId, diag) {
    ...
    abrirModal('nxRcLabelM', cabecera('ti-printer', diag ? 'Imprimir label de diagnóstico' : 'Imprimir label', 'nxRcLabelM') + cuerpo(`
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px"><div class="nxRcFld"><label>Ancho (mm)</label><input id="lr_w" type="number" class="nxRcInput" value="${w}" oninput="window.nxRc.lrPreview()"></div><div class="nxRcFld"><label>Alto (mm)</label><input id="lr_h" type="number" class="nxRcInput" value="${h}" oninput="window.nxRc.lrPreview()"></div><div class="nxRcFld"><label>Barras (px)</label><input id="lr_bh" type="number" class="nxRcInput" min="10" max="60" value="${(()=>{try{return parseInt(localStorage.getItem('studio_lbl_bh'))||22}catch(e){return 22}})()}" oninput="window.nxRc.lrPreview()"></div><div class="nxRcFld"><label>Letra (px)</label><div style="display:flex;align-items:center;gap:6px"><button type="button" class="btn nxRcBtn light" style="width:34px;padding:0" onclick="window.nxRc.lrFuente(-1)">−</button><span id="lr_font_val" style="font-size:11px;min-width:32px;text-align:center">${f}px</span><button type="button" class="btn nxRcBtn light" style="width:34px;padding:0" onclick="window.nxRc.lrFuente(1)">+</button><input id="lr_f" type="hidden" value="${f}"></div></div></div>
  ...
  // Code128-B/C mínimo (solo SVG, sin librerías) para el IMEI del label
  ...
  function code128SVG(texto, alto) {
    const t = String(texto || '').replace(/[^\x20-\x7e]/g, ''); if (!t) return '';
    const digits = /^\d+$/.test(t) && t.length % 2 === 0; const codes = []; let sum;
    if (digits) { codes.push(105); for (let i = 0; i < t.length; i += 2) codes.push(parseInt(t.substr(i, 2), 10)); }
    else { codes.push(104); for (const ch of t) codes.push(ch.charCodeAt(0) - 32); }
    sum = codes[0]; for (let i = 1; i < codes.length; i++) sum += codes[i] * i; codes.push(sum % 103); codes.push(106);
    let x = 0, rects = ''; codes.forEach(c => { const p = C128[c]; for (let i = 0; i < p.length; i++) { const w = +p[i]; if (i % 2 === 0) rects += `<rect x="${x}" y="0" width="${w}" height="${alto}"/>`; x += w; } });
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${x} ${alto}" preserveAspectRatio="none" style="width:100%;height:${alto}px;display:block;shape-rendering:crispEdges" fill="#000">${rects}</svg>`;
  }
  function _lrHtml() {
    ...
    return `<div class="nxRcLabel" style="width:${w}mm;min-height:${h}mm;font-size:${f}px;font-weight:800;line-height:1.15;padding:0.8mm 3mm 0.8mm 4mm;box-sizing:border-box;overflow:hidden;font-family:'Arial Black',Arial,Helvetica,sans-serif;color:#000;background:#fff">
      ...
      ${v('lr_imei') ? `<div style="padding:0.3mm 0">${code128SVG(v('lr_imei'), Math.max(14, parseInt(v('lr_bh')) || 22))}</div>` : ''}
  ...
  function imprimirLabelRapido() {
    ...
    try { localStorage.setItem('studio_lbl_w', w); localStorage.setItem('studio_lbl_h', h); localStorage.setItem('studio_lbl_f', parseInt(v('lr_f')) || 13); localStorage.setItem('studio_lbl_bh', parseInt(v('lr_bh')) || 22); } catch (e) {}
  ...
  async function _cargarImeisDisponibles(productoId, esSerial) {
    const wrap = byId('eq_imei_disp'), sel = byId('eq_imei_sel'); if (!wrap || !sel) return;
    if (!esSerial) { wrap.style.display = 'none'; return; }
    let rows = []; try { rows = await api().get('pos_seriales', 'select=id,serial,color,almacen_id&producto_id=eq.' + productoId + '&estado=eq.disponible&order=created_at.asc&limit=500') || []; } catch (e) {}
    const usados = new Set(cache.refurb.filter(r => r.estado_evaluacion !== 'vendido').map(r => (r.imei || '').toLowerCase()));
    rows = rows.filter(r => !usados.has(String(r.serial || '').toLowerCase()));
    sel.innerHTML = '<option value="">— Elegir un IMEI disponible o escribirlo abajo —</option>' + rows.map(r => `<option value="${esc(r.serial)}" data-color="${esc(r.color || '')}">${esc(r.serial)}${r.color ? ' · ' + esc(r.color) : ''}</option>`).join('');
  ...
  function imprimirReporteCostosLote(soloSel, pdf) {
